...
function GetTree(objs, par, lvl) {
    ...
    $.each(objs, function(k,ele) {
           ...
           str += '<div data-lvl="'+lvl+'" data-id="'+ele.id+'" data-checked="0" '+
...
function convert_json(json) {
    ...
    $.each(jas, function(k, ele) {
           ...
           if(k == "actividades") {
           $.each(ele, function(k, ele2) {
                  str += '{"id":"'+curr_cnt+'", "org_id":"'+ele2.id+'", "name":"'+ele2.nombre+'", "parent":null, "img":"'+ele2.imagenPrincipalMin+'", "enabled":"0", "expand_level": "0"},';
                  curr_cnt++;
                  });

...
jQuery(document).ready(function ()
{
    ...
    xhReq.send(null);
    document.getElementById("page-passwordrecovery").innerHTML=xhReq.responseText;
	...
	xhReq.send(null);
	document.getElementById("page-activitydetails").innerHTML=xhReq.responseText;
	...
	xhReq.send(null);
	document.getElementById("page-installationdetails").innerHTML=xhReq.responseText;
	...
	xhReq.send(null);
	document.getElementById("page-achievementdetails").innerHTML=xhReq.responseText;
	...
	xhReq.send(null);
	document.getElementById("page-milestonedetails").innerHTML=xhReq.responseText;
	...
	xhReq.send(null);
	document.getElementById("popup-settingsfavourites").innerHTML=xhReq.responseText;
...
function onDeviceReady()
{
    ...
    if(window.localStorage.getItem("TOKEN"+idClub) == null && window.localStorage.getItem("CLIENTID"+idClub))
    {
    	isNewUser = true;
    }
...
function onBackKeyDown() 
{
	...
	if ((isPopoverOpening == 'modal-overlay modal-overlay-visible')||(isPopupOpening == 'popup tablet-fullscreen modal-in')){
		myApp.closeModal();
	} else{
		...
		if(myPhotoBrowserPhotoGalleryNewDetails.swiper != undefined){
	    	...
	    	myApp.closePanel();
		} else if(currentPage == 'home' || currentPage == 'update' ||
			currentPage == 'unsupported' || currentPage == 'initsettings' ||
			currentPage == 'initintermediate') {
	    	navigator.app.exitApp();
	    } else if(currentPage == 'login' ||
        			currentPage == 'smscode' || currentPage == 'setprofile'){
        	    	loginBackButton();
        } else if (currentPage == 'news' || currentPage == 'sports' ||
	    			currentPage == 'activities' || currentPage == 'events' ||
	    			currentPage == 'about' ){
...
function builderBannerPublicityList(url,link){
	...
	if(url != ""){
		strBuilderBanner.push('<img data-src="'+url+'" onclick="openBrowser(\''+link+'\')" class="lazy lazy-fadeIn img-list-banner"/>');
	}
	else{
		strBuilderBanner.push('<img class="img-list-banner" src="img/banner-publicidad.png"/>');
	}
...
function builderBannerPublicityDetail(banner){
	...
	if(banner.urlAdBanner != ""){
		strBuilderBanner.push('<img data-src="'+banner.urlAdBanner+'" onclick="openBrowser(\''+banner.linkAdBanner+'\')" class="lazy lazy-fadeIn imgAdBannerBottom"/>');
	}
	else{
		strBuilderBanner.push('<img class="imgAdBannerBottom" src="img\\advertising_01.jpg" alt="Espacio Publicitario"/>');
	}
	return(strBuilderBanner.join(""));
}
